...
. /etc/openvpn/script/config.sh
##Authentication
user_id=$(mysql -h$HOST -P$PORT -u$USER -p$PASS $DB -sN -e "select username from tbl_user where username = '$username' AND subcription in ('VIP','Private','Premium') AND password = '$password' AND user_enable=1 AND user_online=0 AND (now() <= user_end_date)")
##Check user
