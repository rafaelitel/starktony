...
cat /etc/openvpn/keys/ca.crt >> /var/www/html/client.ovpn
echo '</ca>' >> /var/www/html/client.ovpn
yum install zip -y
cd /var/www/html
...
echo "Server Type: $TYPE" >> /root/install.txt
echo "OpenVPN Sun Noload Config: $MYIP/client.zip" >> /root/install.txt
echo $date >> /root/install.txt
cat /root/install.txt
echo "======================================================="
echo "Autoscript created by StarkTOny)"
echo "======================================================="
history -c
exit 0
sleep 3
